...
## Что реализовано и как тестировать
### Этап 1
- Добавлен прототип CLI `second_practic/dependency_cli.py`, который принимает параметры этапа: имя пакета, репозиторий, режим тестового репозитория, версию, режим ASCII-дерева, максимальную глубину и фильтр.
- Все параметры проходят базовую валидацию: проверяются формат имени, версии, глубины, корректность URL и существование файла тестового репозитория.
- При запуске приложение выводит пары ключ-значение со всеми настройками, что позволяет быстро увидеть конфигурацию.

#### Как запускать
- Реальный режим (URL репозитория):
  ```bash
  uv run python second_practic/dependency_cli.py \
    ...
    --filter core
  ```
- Тестовый режим (локальный файл репозитория):
  ```bash
  uv run python second_practic/dependency_cli.py \
    --package sample \
    ...
    --test-mode file
  ```
- Примеры ошибок: попробуйте указать `--test-mode real` вместе с локальным путём или задать отрицательную глубину (`--max-depth -1`) — CLI сообщит о проблеме и завершится с ошибкой.